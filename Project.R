...
library("rsample")
library(tidyr)
fit.rf <- randomForest(Value_Num ~ ., data = train1)
fit.rf
plot(fit.rf)
...
plot(pred.rf,test1$Value_Num, xlab = "Error", ylab = "Value of Player", pch = 3)

+
  #Errors histogram
layout(matrix(c(1),2,2))
hist(err.rf, main="Value_Num", sub="(Actual-Predicted)", xlab="Error", breaks=10, col="darkred")
